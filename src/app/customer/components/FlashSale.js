  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-4">
        <span className="text-[#FF0000] font-semibold">On Sale Now</span>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
        {products.map((product, index) => (
          <div key={index} className="bg-white rounded-lg shadow-lg overflow-hidden text-center p-4 relative transform transition duration-300 hover:scale-105 hover:shadow-xl">
            {product.soldOut && (
              <div className="absolute top-2 right-2 bg-[#FF0000] text-white text-xs px-2 py-1 rounded">SOLD OUT</div>
            )}
            <img src={product.image} alt={product.name} className="w-full h-32 object-cover mb-2" />
            <p className="text-sm">{product.name}</p>
            <p className="text-[#FF0000] font-semibold">Rs.{product.price}</p>
